...
class GameObject(object):
    ...
    def isOutOfBounds(self):
        isOutOfBoundsX = (self.position - Vector2(self.width/2, 0)).x < 0 or (self.position +
                                                                              Vector2(self.width/2, 0)).x > int(OptionsReader().getValue("WindowWidth"))
        isOutOfBoundsY = (self.position - Vector2(0, self.height/2)).y < 0 or (self.position +
                                                                               Vector2(0, self.height/2)).y > int(OptionsReader().getValue("WindowHeight"))
        return isOutOfBoundsX or isOutOfBoundsY

    def isOffScreen(self):
        isOutScreenX = (self.position + Vector2(self.width/2, 0)).x < 0 or (self.position -
                                                                            Vector2(self.width/2, 0)).x > int(OptionsReader().getValue("WindowWidth"))
        isOutScreenY = (self.position + Vector2(0, self.height/2)).y < 0 or (self.position -
                                                                             Vector2(0, self.height/2)).y > int(OptionsReader().getValue("WindowHeight"))
        return isOutScreenX or isOutScreenY

    def move(self):
        if self.movement == Vector2(0, 0):
            return
